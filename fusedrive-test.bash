...
fuse_mount() {
    if [ "$NOMOUNT" -eq 0 ]; then
        ...
        $VALGRIND $VALGRIND_OPTS $EXE "$MOUNTPATH" > /dev/stderr 2>> "$VALGRIND_REDIR" &
        FDPID=$!
...
test_hard_link() {
    ...
    run_test "$RM_ATTEMPTS" "$RM_WAIT" 0 test_rm "$mybasename"
    fuselog Ok
    fuselog -n "Deleting '$dirone/$mybasename'... "
    run_test "$RM_ATTEMPTS" "$RM_WAIT" 0 test_rm "$dirone/$mybasename"
    fuselog Ok
    fuselog -n "Deleting '$dirtwo/$mybasename'... "
    run_test "$RM_ATTEMPTS" "$RM_WAIT" 0 test_rm "$dirtwo/$mybasename"
    fuselog Ok
    fuselog -n "Deleting directory '$dirone'... "
    run_test "$RM_ATTEMPTS" "$RM_WAIT" 0 test_rmdir "$dirone"
    fuselog Ok
    fuselog -n "Deleting directory '$dirtwo'... "
    run_test "$RM_ATTEMPTS" "$RM_WAIT" 0 test_rmdir "$dirtwo"
    fuselog Ok
...
test_rename_clobber() {
    ...
    run_test "$RM_ATTEMPTS" "$RM_WAIT" 0 test_rm "$newfilename"
...
if run_test "$RM_ATTEMPTS" "$RM_WAIT" 0 test_rm "$ROOTFILENAME"; then
    fuselog Ok
else
    fuselog Failed, continuing on.
fi
...
if run_test "$RM_ATTEMPTS" "$RM_WAIT" 0 test_rm "${NEWDIRNAME}/${SUBFILENAME}"; then
    fuselog Ok
else
    fuselog Failed, continuing on.
fi
...
if run_test "$RM_ATTEMPTS" "$RM_WAIT" 0 test_rmdir "$NEWDIRNAME"; then
    fuselog Ok
else
    fuselog Failed, continuing on.
fi
...
if ! run_test "$DEFAULT_ATTEMPTS" "$DEFAULT_WAIT" 0 test_mkdir; then
    ...
    fuselog -n "Cleaning up by deleting '$DIRNAME' (will say Ok regardless of success)... "
        run_test "$RM_ATTEMPTS" "$RM_WAIT" 0 test_rmdir "$DIRNAME"
        fuselog Ok
    unset DIRNAME
fi
